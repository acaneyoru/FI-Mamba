...
class ReferenceSelectiveSSM(nn.Module):
    """
    Readable diagonal selective state-space layer.

    For each token t and channel c:

        h_t = a_t * h_(t-1) + b_t * u_t
        y_t = c_t * h_t + d * u_t

    a_t, b_t, and c_t are input-dependent. The default implementation uses a
    vectorized diagonal recurrence with numerical history truncation. It is a
    method scaffold, not a replacement for an optimized Mamba selective scan.
    """
    ...
    def __init__(
        ...
        self.in_projection = nn.Linear(dim, inner_dim * 2)
        ...
        self.log_a = nn.Parameter(torch.full((inner_dim,), -2.0))
        self.skip = nn.Parameter(torch.ones(inner_dim))
        self.out_projection = nn.Linear(inner_dim, dim)
        self.dropout = nn.Dropout(dropout)

    @staticmethod
    def _parallel_diagonal_scan(decay: Tensor, drive: Tensor) -> Tensor:
        """
        Parallel form of h_t=a_t*h_(t-1)+drive_t.

        Very old contributions are numerically truncated after the accumulated
        log decay reaches -30. This keeps the transparent PyTorch reference
        stable; production code should use a fused selective-scan kernel.
        """
        log_decay = torch.log(decay.clamp(min=1e-5, max=1.0))
        log_prefix = torch.cumsum(log_decay, dim=1)
        safe_prefix = log_prefix.clamp(min=-30.0, max=30.0)
        scaled_drive = drive * torch.exp(-safe_prefix)
        return torch.exp(safe_prefix) * torch.cumsum(scaled_drive, dim=1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        state = self._parallel_diagonal_scan(decay, delta * input_map * value)
...
class IsotropicUnbiasedScan(nn.Module):
    ...
    def __init__(self, dim: int, expansion: int = 2, dropout: float = 0.0) -> None:
        super().__init__()
        self.ssm = ReferenceSelectiveSSM(dim, expansion=expansion, dropout=dropout)
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        forward_state = self.ssm(x)
        backward_state = torch.flip(self.ssm(torch.flip(x, dims=(1,))), dims=(1,))
